...
//   m_search->setListView(m_list);
  ...
  //connect(m_search, SIGNAL(startedSearch(const QString&)));
...
//     KSData::instance()->getDictionary()->addQueue(KSQuery(INDEX));
    //temp=KSData::instance()->getDictionary()->processList("SELECT name, id, search FROM phrases ORDER BY search ASC;", 3);
    /*if(!KSData::instance()->getDictionary()->processIndex())
    {
      KMessageBox::error(this, i18n("Couldn't created index of words!"));
      return;
  }*/
